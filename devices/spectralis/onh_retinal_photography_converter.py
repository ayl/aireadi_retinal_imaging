    ...
    sequences=[
        ...
        ElementList(
            ...
            [
                ...
                Element(
                    "CodeMeaning", "00080104", "LO", HARMONIZE, "Optic nerve head"
                ),
            ],
        ),
        ElementList(
...
def extract_dicom_dict(file, tags):
    ...
    seq = getattr(dataset, "PatientEyeMovementCommandCodeSequence", None)
    if seq and len(seq) > 0:
        item = seq[0]
        item.CodingSchemeDesignator = "SCT"  

        lat = getattr(dataset, "ImageLaterality", None)
        if lat == "R":
            item.CodeValue = "255531009"
            item.CodeMeaning = "Right gaze"
        elif lat == "L":
            item.CodeValue = "255530005"
            item.CodeMeaning = "Left gaze"

    # dataset.PatientEyeMovementCommandCodeSequence[0].CodingSchemeDesignator = "SCT"

    # if dataset.ImageLaterality == "R":
    #     dataset.PatientEyeMovementCommandCodeSequence[0].CodeValue = "255531009"
    #     dataset.PatientEyeMovementCommandCodeSequence[0].CodeMeaning = "Right gaze"

    # elif dataset.ImageLaterality == "L":
    #     dataset.PatientEyeMovementCommandCodeSequence[0].CodeValue = "255530005"
    #     dataset.PatientEyeMovementCommandCodeSequence[0].CodeMeaning = "Left gaze"
...
def write_dicom(protocol, dicom_dict_list, file_path):
    ...
    for key in keys:
        ...
        if key in dicom_dict_list[0]:
            ...
            setattr(dataset, element_name, value)
    dcmwrite(file_path, dataset, write_like_original=False)
