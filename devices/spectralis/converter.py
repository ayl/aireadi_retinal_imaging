...
from . import onh_oct_converter as spectralis_onh_oct_converter
from . import onh_retinal_photography_converter as spectralis_onh_retinal_photography_converter
from . import ppol_oct_converter as spectralis_ppol_oct_converter
from . import ppol_retinal_photography_converter as spectralis_ppol_retinal_photography_converter
...
def convert_octa_dicom(input, outputfolder):
    """
    Converts a DICOM file to a specific format based on its type and saves the converted file to an output folder.
    ...
    if "_op.dcm" in input:
        dic = spectralis_octa_retinal_photography_converter.convert_dicom(
            input, outputfolder
        )
    elif "_opt.dcm" in input:
        dic = spectralis_octa_oct_converter.convert_dicom(input, outputfolder)

    elif "_vol.dcm" in input:
        dic = spectralis_octa_oct_converter.convert_dicom(input, outputfolder)

    elif "_heightmap_smooth.dcm" in input:
        dic = spectralis_octa_heightmap_converter.convert_dicom(
            input, outputfolder
        )
    elif "_enface.dcm" in input:
        dic = spectralis_octa_enface_converter.convert_dicom(
            input, outputfolder
        )
